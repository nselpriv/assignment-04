...
public class TaskRepository : ITaskRepository
{
    ...
    public (Response Response, int TaskId) Create(TaskCreateDTO task)
    {
        ...
        Collection<Tag> assignedTags = new();

        // Create the associated tags
        foreach (String tag in task.Tags)
        {
            var tagToAdd = _context.Tags.FirstOrDefault(c => c.Name == tag) ?? new Tag(tag);

            if (tagToAdd is null) _context.Tags.Add(tagToAdd);
    ...
    public Response Delete(int taskId)
    {
        ...
        if (deleteDis.state == State.Active)
        {
            ...
            , State.Removed, deleteDis.created));
    ...
    public Response Update(TaskUpdateDTO task)
    {
        ...
        updateDis.statusUpdated = DateTime.UtcNow;
